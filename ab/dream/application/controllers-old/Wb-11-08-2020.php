...
class Wb extends REST_Controller
{
	...
	{		
		...
		if(!$chkoffdate && $user_master_id!="" && count($play_data)>0 && $totalbids >= 10 && $getcurrent_balance >= $totalbids && ($user_data->user_type==0 or $user_data->user_type==1) && $draw_master_id >= $currdrawmasterid && $draw_master_id>0)
		{
			$insert_data = array(
	...
	function cancle_transaction_post()
	{
		...
		$countdeletes = $this->common_model->getAllwhere('draw_transaction_master',array('is_result'=>date('Y-m-d'),'is_deleted'=>'1','bar_code_number'=>$bar_code_number));
		...
		{
			$response= array('status'=>'201', 'message'=>'Alrady Cancle This Transaction.', 'data'=>null);	
		}
		else if($ctime > $en_date_time)
		{
			$response= array('status'=>'201', 'message'=>'Cancle Transaction End.', 'data'=>null);	
		}
		else if($results)
		{
			$response= array('status'=>'201', 'message'=>'Result Declared.', 'data'=>null);	
		}
		else if($countdeletes>$setting->cancle_per_day_limit)
		{
			$response= array('status'=>'201', 'message'=>'Today Your Cancle Limit Finished.', 'data'=>null);	
		}
		else{
			...
			{ 
				...
				$response= array('status'=>'200', 'message'=>'Transaction Cancled Successfully.', 'data'=>null);
			}else{
	...
	function claim_post()
	{
		...
		else if($bar_code_data->is_claim=="1")
		{
			...
			$response= array('status'=>'201', 'message'=>'Claim already settled on '.$claim_date_time.'.', 'data'=>null);	
		}
	...
	{		
		...
		else
		{
			...
			if(count($transactions)>0){ 
				...
				$total_comission = $this->wb_model->total_comission_new($tr->user_master_id,$fromdate,$todate);
	...
	function result_time_post()
	{
		...
		else
		{
			...
			$response= array('status'=>'201', 'newdate'=>$newdate, 'message'=>'Rsults Time Get Successfully.', 'data'=>null);
		}
