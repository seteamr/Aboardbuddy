...
class Wb extends REST_Controller
{
	...
	{		
		...
		if($chkoffdate)
		{
			$response= array('status'=>'201', 'message'=>'Server Maintenance Suru aslyamule system band aahe.', 'data'=>null);	
		}
	...
	{		
		...
		$result_date	= Date('Y-m-d'); //$this->post('result_date');
		$draw_master_ids	= $this->post('draw_master_id');
		...
		foreach($draw_master_ids as $draw_master_id)
		{
			...
			{
				$countinue++;
				$response= array('status'=>'201', 'message'=>'Market Close. Please login next day between 10am to 10pm', 'data'=>null);	
			}
			...
			if($last10seconds)
			{
				...
				{
					$countinue++;
					$response= array('status'=>'201', 'message'=>'Draw Time finish', 'data'=>null);
				}
			}
	...
	function cancle_transaction_post()
	{
		...
		{
			$response= array('status'=>'201', 'message'=>'Current Draw is Over. Can not Calcel Ticket.', 'data'=>null);	
		}
		else if($results)
		{
	...
	function claim_post()
	{
		...
		if($user_master_id=='')
		{
			$response= array('status'=>'201', 'message'=>'User Required.', 'current_balance'=> $getcurrent_balance, 'data'=>null);	
		}
		else if(!$user_data)
		{
			$response= array('status'=>'201', 'message'=>'User Not Exist.', 'current_balance'=> $getcurrent_balance, 'data'=>null);	
		}
		else if($bar_code_number=='')
		{
		...
		{
			$response= array('status'=>'201', 'message'=>'Bar Code Not Exist OR Bar Code and User not match.', 'current_balance'=> $getcurrent_balance, 'data'=>null);
		}
		else if($bar_code_data->is_deleted=="1")
		{
			$response= array('status'=>'201', 'message'=>'This Ticket Is Already Canceled', 'current_balance'=> $getcurrent_balance, 'data'=>null);	
		}
		else if($bar_code_data->is_claim=="1")
		{
			...
			if(count($play_data)>0)
			{
				...
				$response= array('status'=>'201', 'message'=>'TICKET ALREADY CLAIMED ON '.$claim_date_time, 'current_balance'=> $getcurrent_balance, 'data'=>null);
			}else{
