	...
	{ 
		...
		foreach($records as $r)
		{
			...
			{ 
				...
				//=======Start Give winning to user whose winning is less than 60% of days sales=============
				//
				/*
						JOIN	user_master um
						ON		um.user_master_id = dtd1.user_master_id AND	um.winning_distribution = 0	
				*/
				$points_used_for_60_percentage = 0;
				$sql5a =
				"SELECT	dtdinline.user_master_id
				,		ROUND(SUM(bid_units*bid_points*bid_points_multiplier)) AS Sales
				,		ROUND(SUM(bid_units*bid_points*bid_points_multiplier)* ".$setting->distribute_per."/100) AS Sales_distribution
				,		ROUND(SUM(IF(is_winning=1, bid_units*bid_points*bid_points_multiplier*90,0))) AS win_Amount
				,		(SUM(bid_units*bid_points*bid_points_multiplier)* 60/100) - ROUND(SUM(IF(is_winning=1, bid_units*bid_points*bid_points_multiplier*90,0))) AS advance_total_bid_points
				...
				AND		dtdinline.draw_master_id BETWEEN 1 AND '48'
				...
				HAVING	ROUND(SUM(IF(is_winning=1, bid_units*bid_points*bid_points_multiplier*90, 0)))/ROUND(SUM(bid_units*bid_points*bid_points_multiplier))*100 <=60
				AND		(SUM(bid_units*bid_points*bid_points_multiplier)* ".$setting->distribute_per."/100) - ROUND(SUM(IF(is_winning=1, bid_units*bid_points*bid_points_multiplier*90,0))) >= 180
				...
				$num5a = 0;
				$num5a = $r5a->num_rows();
				$last_query =  "/*Query for Less than 60%*/" . " " . $this->db->last_query();
				...
				if($num5a>0)
				{
					...
					for($j=0;$j<count($result5a);$j++)
					{
						...
						$Sales = $result5a[$j]->Sales;
						$win_Amount = $result5a[$j]->win_Amount;
						
						if(($win_Amount - $Sales) <10000){
						...
						$advance_total_bid_points = $result5a[$j]->advance_total_bid_points;
						//$last_query =  "/* advance_total_bid_points, Block: */ " . $j . " Points: " . $advance_total_bid_points;
						//$this->common_model->insertData('q_info',array('info'=>$last_query));
						...
						ORDER BY 	number_of_bids DESC, total_bid_points DESC, bid_points DESC
						LIMIT 10";

						$r5 = $this->db->query($sql5);
						$result5 = $r5->result();
						$num5 = $r5->num_rows();
						$last_query = "/*INNER : Query for Less than 60 */ " . " " .  $this->db->last_query();
						$this->common_model->insertData('q_info',array('info'=>$last_query));
						...
						if($num5>0)
						{
							for($i=0;$i<count($result5);$i++)
							{
								...
								//$chkexist = $this->common_model->check_bid_akada_number($result_date,$draw_master_id,$bid_akada_number,$series_master_id,$bajar_master_id);
								...
								{    					                    											
									...
									{ 
										...
										$points_used_for_60_percentage = $points_used_for_60_percentage + $winning_points;
										$ins_data = array(
											'result_date' 	 	=> $result_date,
											'draw_master_id' 	=> $draw_master_id,
				...
				//=======End Give winning to user whose winning is less than 60% of days sales=============
				$last_query =  "/*Winnning given to less than 60% */" . " " . $points_used_for_60_percentage;
				$this->common_model->insertData('q_info',array('info'=>$last_query));
				
				/////////////////////////////////////////////////////////////////////
				//original draw_transaction_details
				$sql =
				"SELECT	SUM(dtd.bid_units*dtd.bid_points_multiplier*dtd.bid_points)* ".$setting->distribute_per."/100  AS total_bid_points
				...
				$draw_master_id_last3 = $draw_master_id-4;
				$draw_master_id_last2 = $draw_master_id-1;
				
				$prevq =	
				"SELECT SUM(bid_units*bid_points_multiplier*bid_points)* ".$setting->distribute_per."/100 AS old_sales_balance
				FROM 	draw_transaction_details dtd
				JOIN	user_master um
				ON		um.user_master_id = dtd.user_master_id AND	um.winning_distribution = 0	
				WHERE	dtd.result_date = '".$result_date."'
				...
				$last_query =  "/*Collection after less than 60% */" . " " . $total_collection1;
				$this->common_model->insertData('q_info',array('info'=>$last_query));
				/////////////////////////////////////////////////////////////////////
				
				//$total_collection1 =  $total_collection1 - $points_used_for_60_percentage ;
				//$last_query =  "/*Total Collection after less than 60% */" . " " . $total_collection1;
				//$this->common_model->insertData('q_info',array('info'=>$last_query));
				
				//=====Start Winning for Bigger bidding numbers=======================
				$block_points3 = $total_collection1*0.4;
				
				$last_query =  "Winning for less than 60 collective" . " " . $block_points3;
				$this->common_model->insertData('q_info',array('info'=>$last_query));
				...
				ON			bid_master.draw_transaction_master_id = dtd.draw_transaction_master_id
				WHERE		dtd.result_date =  '".$result_date."'
				AND			dtd.draw_master_id ='".$draw_master_id."' 
				AND			dtd.is_deleted = 0
				...
				$num3 = $r3->num_rows();
				$last_query =  "/* Winning for less than 60 : 3rd block*/ " . " " . $this->db->last_query();
				$this->common_model->insertData('q_info',array('info'=>$last_query));
				...
				$last_query =  "Balance After less than 60" . " " . $block_points3;
				$this->common_model->insertData('q_info',array('info'=>$last_query));				
				...
				$last_query =  "Balance for Smaller Bidding" . " " . $block_points6;
				$this->common_model->insertData('q_info',array('info'=>$last_query));
				...
				AND			dtd.user_master_id IN 
					(
						...
						GROUP BY dtd1.user_master_id
						HAVING	(SUM( dtd1.bid_units*dtd1.bid_points*dtd1.bid_points_multiplier)* ".$setting->distribute_per."/100) - ROUND(SUM(IF(dtd1.is_winning=1, dtd1.bid_units*dtd1.bid_points*dtd1.bid_points_multiplier*90,0)))>= 180
						AND		ROUND(SUM(IF(is_winning=1, bid_units*bid_points*bid_points_multiplier*90, 0)))/ROUND(SUM(bid_units*bid_points*bid_points_multiplier))*100 <= ".$setting->distribute_per."/1
					)
				GROUP BY 	dtd.result_date
				,			dtd.draw_master_id
				,			dtd.series_master_id
				...
				AND         SUM(dtd.bid_units*dtd.bid_points_multiplier*dtd.bid_points) >= 2
				ORDER BY 	number_of_bids DESC, total_bid_points DESC, bid_points
				LIMIT 20";
				...
				,			ROUND((SUM(bid_units*bid_points*bid_points_multiplier))*um.winning_distribution/100) - ROUND(SUM(IF(is_winning=1, bid_units*bid_points*bid_points_multiplier*90, 0))) AS total_diff
				FROM	draw_transaction_details dtd
				JOIN	user_master um
				ON		um.user_master_id = dtd.user_master_id AND	um.winning_distribution > 0	
				...
				HAVING	MIN(bid_units*bid_points*bid_points_multiplier)*90 <= ROUND((SUM(bid_units*bid_points*bid_points_multiplier))*um.winning_distribution/100) - ROUND(SUM(IF(is_winning=1, bid_units*bid_points*bid_points_multiplier*90, 0)))
				AND	ROUND((SUM(bid_units*bid_points*bid_points_multiplier))*um.winning_distribution/100) - ROUND(SUM(IF(is_winning=1, bid_units*bid_points*bid_points_multiplier*90, 0)))>=180
				ORDER BY ROUND((SUM(bid_units*bid_points*bid_points_multiplier))*um.winning_distribution/100) - ROUND(SUM(IF(is_winning=1, bid_units*bid_points*bid_points_multiplier*90, 0)))
				DESC
				LIMIT 5";
				...
				if($num4aAdvanceDraw>0)
				{
					for($j=0;$j<count($result4aAdvanceDraw);$j++)
					{
						...
						AND			COUNT(tdtd.user_master_id) = 1
						AND			SUM(bid_units*bid_points_multiplier*bid_points) >= 2
						ORDER BY 	number_of_bids DESC, total_bid_points DESC, bid_points
						LIMIT 5";
						
						//$last_query =  "/*Advance Draw String block inner*/ " . " " . $sql4Advanced;
						//$this->common_model->insertData('q_info',array('info'=>$last_query));
				...
				if($num4a>0)
				{
					for($j=0;$j<count($result4a);$j++)
					{
						...
						FROM 	draw_transaction_details tdtd
						WHERE	result_date = '".$result_date."'
						AND		draw_master_id = '".$draw_master_id_100."'
						AND		series_master_id = '".$series_master_id_100."'
						AND		bajar_master_id = '".$bajar_master_id_100."'
						GROUP BY result_date
						,		draw_master_id
						,		series_master_id
						,		bajar_master_id
				...
				// Original draw_transaction_details
				$sqlupd =
				"UPDATE     draw_transaction_details DTD SET DTD.is_winning = 1
				WHERE 		DTD.result_date = '".$result_date."' 
				AND 		DTD.draw_master_id = '".$draw_master_id."'
				AND			DTD.is_winning = 0
				...
				(
						SELECT	1
						FROM	result_master RM
						WHERE	RM.result_date = DTD.result_date
						AND		RM.draw_master_id = DTD.draw_master_id
				...
				// Original draw_transaction_details
				$this->common_model->updateData('draw_transaction_details',array('is_result'=>'1','winning_per'=>$setting->distribute_per),array('result_date'=>$result_date,'draw_master_id'=>$draw_master_id));
				...
				{											
					...
					/*$sqlsetting = "UPDATE	setting 
						SET		distribute_per = 0
						WHERE 	id = '1'";
					$qqsetting = $this->db->query($sqlsetting);*/
				}
				////////////////////////////////////////
				// Update Duplicate Result 
				$sqlupdate_double =
				"UPDATE		result_master 
				SET			is_result_declare = 3
				WHERE 		result_date  = '".$result_date."' 
				AND			draw_master_id  = '".$draw_master_id."'
				...
				$this->common_model->updateData('result_master',array('is_result_declare'=>'1','balance'=>$block_points6),array('result_date'=>$result_date,'draw_master_id'=>$draw_master_id));
