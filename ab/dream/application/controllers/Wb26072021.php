...
class Wb extends REST_Controller
{
	...
	function logout_post()
	{
		...
		if($user_master_id=='')
		{
			$response= array('status'=>'201', 'message'=>'Client Required.', 'data'=>null);	
		}		
		else
		{
			...
			{ 
				...
				$response= array('status'=>'200', 'message'=>'Logout Successfully. Done', 'data'=>null);
			}
			else
			{
				$response= array('status'=>'201', 'message'=>'Client Not Exist.', 'data'=>null);	
			}
		}
		$this->response($response	, 200); // 200 being the HTTP response code	
	}
	...
	function login_post()
	{
		...
		else
		{
			...
			{
				$response= array('status'=>'201', 'message'=>'Password OR Client OR Device Token Not match.', 'data'=>null);	
			}
		}
		$this->response($response	, 200); // 200 being the HTTP response code	
	}
	...
	function login1_post()
	{
		...
		else
		{
			...
			if($error=="0")
			{
				...
				{
					$response= array('status'=>'201', 'message'=>'Password OR Client Code OR Device Id Not Avtive.', 'data'=>null);	
				}
			}
		}
		$this->response($response, 200); // 200 being the HTTP response code
	...
	{		
		...
		if($getcurrent_balance < $totalbids*$number_of_bids)
		{
			$countinue++;
			$response= array('status'=>'201', 'message'=>'Your Coins Balance is Low.', 'data'=>null);
		}
		...
		foreach($draw_master_ids as $draw_master_id)
		{
			...
			$points_transferred = ($p['bid_units']*$p['bid_points']*$p['bid_points_multiplier']); //$totalbids;
			$opening_points 	= $getcurrent_balance;
			$closing_points		= $getcurrent_balance - $points_transferred;
	...
	function cancel_transaction_banana_post()
	{
		...
		{
			$response= array('status'=>'201', 'message'=>'Current Draw is Over. Can not Calcel Coupon.', 'data'=>null);	
		}
		else if($results)
		{
		...
		else if(count($countdeletes)>=$setting->cancle_per_day_limit)
		{
			...
			{ 
				...
				$response= array('status'=>'200', 'message'=>'Transaction Canceld Successfully.', 'current_balance'=> $getcurrent_balance);
			}else{
	...
	function claim_post()
	{
		...
		if($user_master_id=='')
		{
			$response= array('status'=>'201', 'message'=>'Client Required.', 'current_balance'=> $getcurrent_balance, 'data'=>null);	
		}
		else if(!$user_data)
		{
			$response= array('status'=>'201', 'message'=>'Client Not Exist.', 'current_balance'=> $getcurrent_balance, 'data'=>null);	
		}
		else if($bar_code_number=='')
		{
			$response= array('status'=>'201', 'message'=>'Bar Code Required.', 'current_balance'=> $getcurrent_balance, 'data'=>null);	
		}
		else if(!$bar_code_data)
		{
			$response= array('status'=>'201', 'message'=>'Bar Code Not Exist OR Bar Code and Client not match.', 'current_balance'=> $getcurrent_balance, 'data'=>null);
		}
		else if($bar_code_data->is_deleted=="1")
		{
			$response= array('status'=>'201', 'message'=>'This Coupon Is Already Canceled', 'current_balance'=> $getcurrent_balance, 'data'=>null);	
		}
		else if($bar_code_data->is_claim=="1")
		{
			...
			if(count($play_data)>0)
			{
				...
				$response= array('status'=>'201', 'message'=>'Coupon ALREADY CLAIMED ON '.$claim_date_time, 'current_balance'=> $getcurrent_balance, 'data'=>null);
			}else{
				//$response= array('status'=>'201', 'message'=>'Claim already settled on '.$claim_date_time.'. No Winning amount is '.$totalbids.'.', 'data'=>null);
				$response= array('status'=>'201', 'message'=>'No Winning. Hard luck. Play Again To Win More.');	
			}
		}
		else if($ctime < $entime )
		{
			$response= array('status'=>'201', 'message'=>'Current Draw is Not Over.', 'current_balance'=> $getcurrent_balance, 'data'=>null);	
		}
		else if($bar_code_data->is_result==0)
		{
			$response= array('status'=>'201', 'message'=>'Please claim after some time.', 'current_balance'=> $getcurrent_balance, 'data'=>null);	
		}
		else if($ctime > $addtendays )
		{
			$response= array('status'=>'201', 'message'=>$ctime .'-'.$addtendays.'Coupon Expired, Claim Date/Time Over.', 'current_balance'=> $getcurrent_balance, 'data'=>null);	
		}
		else
		{
			$play_data = $this->wb_model->getAllwhere('draw_transaction_details',array('draw_transaction_master_id'=>$bar_code_data->draw_transaction_master_id,'is_winning'=>'1'));			
			...
			if(count($play_data)>0)
			{
				...
				if($totalbids==0)
				{
					$response= array('status'=>'200', 'message'=>'No Winning. Better luck. Try Next time.', 'current_balance'=> $getcurrent_balance, 'data'=>null);	
				}else{
					$getcurrent_balance = $this->getcurrent_balance($user_master_id);
					//$response= array('status'=>'201', 'message'=>'Already Cancel This Transaction.', 'current_balance'=> $getcurrent_balance);
					
					$response= array('status'=>'200', 'message'=>'Congratulations You Win '.$totalbids.'/-!!!', 'current_balance'=> $getcurrent_balance, 'data'=>$final_d);
				}
			}else{
				$this->common_model->updateData('draw_transaction_details',array('is_claim'=>'1'),array('draw_transaction_master_id'=>$bar_code_data->draw_transaction_master_id));				
				$this->common_model->updateData('draw_transaction_master',array('claim_date_time'=>date('Y-m-d H:i:s'),'is_claim'=>'1','claim_user_master_id'=>$user_master_id),array('bar_code_number'=>$bar_code_number));
	...
	function new_claim_post()
	{
		...
		else if($bar_code_data->is_claim=="1")
		{
			...
			if(count($play_data)>0)
			{
				...
				//$response= array('status'=>'201', 'message'=>'Claim already settled on '.$claim_date_time.'. No Winning amount is '.$totalbids.'.', 'data'=>null);
				$response= array('status'=>'201', 'message'=>'No Winning. Better luck. Try Next time.');	
			}
		}
		else if($ctime < $entime )
		{
			$response= array('status'=>'201', 'message'=>'Currently draw not end.', 'current_balance'=> $getcurrent_balance, 'data'=>null);	
		}
		else if($bar_code_data->is_result==0)
		{
			$response= array('status'=>'201', 'message'=>'Under Proccess result Please claim after some time.', 'current_balance'=> $getcurrent_balance, 'data'=>null);	
		}
		else if($ctime > $addtendays )
		{
			$response= array('status'=>'201', 'message'=>$ctime .'-'.$addtendays.'Not Eligible, claim time expired.', 'current_balance'=> $getcurrent_balance, 'data'=>null);	
		}
		else
		{
			$play_data = $this->wb_model->getclaimdatat($bar_code_number);
	...
	function draw_time_post()
	{
		...
		if($user_master_id=='')
		{
			$response= array('status'=>'201','current_date_time'=>$current_date_time, 'message'=>'Client Required.', 'data'=>null);	
		}
		else if($type=='')
		{
		...
		{
			$response= array('status'=>'201','current_date_time'=>$current_date_time, 'message'=>'Client Not Exist.', 'data'=>null);	
		}		
		else
		{
			if($type=="0")
