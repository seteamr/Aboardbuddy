...
class Wb extends REST_Controller
{
	...
	function cancel_transaction_banana_post()
	{
		...
		else if(count($countdeletes)>=$setting->cancle_per_day_limit)
		{
			...
			if(count($play_data)>0)
			{ 
				$this->common_model->updateData('draw_transaction_master', array('is_deleted'=>'1', 'ca_ipaddress'=>$ipaddress, 'ca_device_token'=>$device_token), array('bar_code_number'=>$bar_code_number));
	...
	function claim_post()
	{
		...
		{
			$response= array('status'=>'201', 'message'=>'Current Draw Is Under Process.', 'current_balance'=> $getcurrent_balance, 'data'=>null);	
		}
		else if($bar_code_data->is_result==0)
		{
			$response= array('status'=>'201', 'message'=>'Current Draw Is Under Process. Please Claim after some time.', 'current_balance'=> $getcurrent_balance, 'data'=>null);	
		}
		...
		else
		{
			...
			if(count($play_data)>0)
			{
				...
				$this->common_model->updateData('draw_transaction_master',array('is_winning'=>'1', 'cl_ipaddress'=>$ipaddress, 'cl_device_token'=>$device_token),array('bar_code_number'=>$bar_code_number));
	...
	function new_claim_post()
	{
		...
		{
			$response= array('status'=>'201', 'message'=>'This draw is Under Process. Wait For Result', 'current_balance'=> $getcurrent_balance, 'data'=>null);	
		}
		else if($bar_code_data->is_result==0)
		{
			$response= array('status'=>'201', 'message'=>'Result Will Declare Soon. Please Try After Some Time.', 'current_balance'=> $getcurrent_balance, 'data'=>null);	
		}
		else if($ctime > $addtendays )
		{
			$response= array('status'=>'201', 'message'=>$ctime .'-'.$addtendays.'No Claim Allowed After Claim Date/Time Is Over', 'current_balance'=> $getcurrent_balance, 'data'=>null);	
		}
		else
		{
			$play_data = $this->wb_model->getclaimdatat($bar_code_number);
			...
			if(count($play_data)>0)
			{
				...
				$this->common_model->updateData('draw_transaction_details',array('is_claim'=>'1'),array('draw_transaction_master_id'=>$bar_code_data->draw_transaction_master_id));				
				$this->common_model->updateData('draw_transaction_master',array('claim_date_time'=>date('Y-m-d H:i:s'), 'is_claim'=>'1','claim_user_master_id'=>$user_master_id, 'cl_ipaddress'=>$ipaddress, 'cl_device_token'=>$device_token), array('bar_code_number'=>$bar_code_number));
		...
		else
		{
			...
			if($draw_master_id!='' && count($draw_data_details)=="100")
			{
				...
				foreach($results as $r)
				{
					...
					$dup_data = $this->wb_model->getsingle('result_master',array('draw_master_id'=>$r->draw_master_id,'series_master_id'=>$r->series_master_id,'bajar_master_id'=>$r->bajar_master_id,'is_result_declare'=>'2'));																				
					if($dup_data && $dup_data->bid_akada_number!=$r->bid_akada_number)
					{
						$newdable = $r->bid_akada_number.$dup_data->bid_akada_number;
					}else{
						$newdable = $r->bid_akada_number;
					}
